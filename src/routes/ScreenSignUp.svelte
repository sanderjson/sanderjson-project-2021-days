<script>
  ...
  const handleSignUp = async () => {
    ...
    const fetchOptions = {
      ...
      body: JSON.stringify({
        ...
        habit: user.habit,
        habitType: user.habitType,
        habitCategory: user.habitCategory,
        habitDateStartISOString: user.habitDateStartISOString,
        habitDateStartUTCString: user.habitDateStartUTCString,
        habitDateEndISOString: user.habitDateEndISOString,
        habitDateEndUTCString: user.habitDateEndUTCString
      })
    };
  ...
  onMount(() => {
    ...
    user.habitDateStartISOString = dateStart.toISOString();
    user.habitDateStartUTCString = dateStart.toUTCString();
    user.habitDateEndISOString = dateEnd.toISOString();
    user.habitDateEndUTCString = dateEnd.toUTCString();
    // console.log("user.habitDateStartISOString", user.habitDateStartISOString);
    // console.log("user.habitDateStartUTCString", user.habitDateStartUTCString);
    // console.log("user.habitDateEndISOString", user.habitDateEndISOString);
  ...
  <form class="space-y-6" on:submit|preventDefault={handleSignUp}>
    ...
    <div class="flex items-center">
      ...
      <button
        ...
        class:bg-indigo-600={user.habitType}
        ...
        <span
          aria-hidden="true"
          class:translate-x-5={user.habitType}
          class="translate-x-5 inline-block h-5 w-5 rounded-full bg-white shadow
          transform ring-0 transition ease-in-out duration-200" />
      </button>
      <span class="ml-3" id="toggleLabel">
        <span class="text-sm font-medium text-gray-900">
          {#if user.habitType}Start a new habit{:else}Kick an old habit{/if}
        </span>
